  ...
  <q-page style="padding:24px">
    ...
    <div style="background:#fff;border:1px solid #d4cfc4;border-radius:12px;padding:12px 16px;margin-bottom:18px;display:flex;flex-wrap:wrap;gap:10px;align-items:center">
      ...
      <div style="margin-left:auto;display:flex;gap:10px">
        <div v-for="u in UBICS" :key="u.key" :style="{background:u.bg,border:`1px solid ${u.border}`,borderRadius:'8px',padding:'5px 12px',textAlign:'center'}">
    ...
    <!-- Groups by ubicacion -->
    <div v-for="ubic in UBICS" :key="ubic.key" style="margin-bottom:22px">
    ...
    <!-- Move -->
    <q-dialog v-if="modal?.mode==='move'" :model-value="true" @hide="modal=null">
      <q-card style="width:440px;max-width:95vw;border-radius:14px;padding:28px">
        ...
        <MoveForm :item="modal.item" @save="onMove" @cancel="modal=null"/>
      </q-card>
    </q-dialog>
...
function onMove(cant, ubic, nota) { store.moveStock(modal.value.item.id, ubic, cant, nota); modal.value = null }
